...
# Setup env for the async setup cooldown
.globals <- new.env()
.globals$last_setup_time <- NULL
.globals$setup_lock <- FALSE

# Class for managing database connections in the async setup
    ...
    request_setup = function(session_id) {
      ...
      tryCatch({
        if (!private$check_cooldown()) {
          ...
          message(sprintf("[Session %s] Async setup cooling down (%d seconds remaining)", 
